...
function Explore() {
  ...
  const [posts, setPosts] = useState([]), [viewIndex, setViewIndex] = useState(null), [query, setQuery] = useState(""), [results, setResults] = useState([]);

  useEffect(() => { fetchExplorePosts().then(r => setPosts(r.data)).catch(console.error); }, []);

  useEffect(() => {
    const t = setTimeout(() => {
      if (query.length > 1) searchUsers(query).then(r => setResults(r.data)).catch(console.error);
      else setResults([]);
    }, 300);
  ...
  return (
    <div className="w-full relative px-2">
      <div className="max-w-xl mx-auto mb-8 relative">
        ...
        {results.length > 0 && <div className="absolute top-full left-0 right-0 mt-2 bg-white border rounded-xl shadow-2xl z-50">
          {results.map(u => (
            <div key={u._id} onClick={() => navigate(`/profile/${u.username}`)} className="flex items-center gap-4 p-4 hover:bg-gray-50 cursor-pointer border-b last:border-none">
              ...
              <div className="flex flex-col"><span className="text-sm font-bold">{u.username}</span><span className="text-xs text-gray-500">{u.bio || "No bio"}</span></div>
            </div>
          ))}
        </div>}
      </div>
      ...
      <div className="grid grid-cols-3 gap-0.5 md:gap-6">
        {posts.map((p, i) => (
          <div key={p._id} onClick={() => setViewIndex(i)} className="relative aspect-square cursor-pointer overflow-hidden rounded-none md:rounded-xl group">
            ...
            <div className="absolute inset-0 bg-black/30 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-4 text-white font-bold">
              <div className="flex items-center gap-1"><FavoriteIcon />{p.likes?.length || 0}</div>
              <div className="flex items-center gap-1"><ChatBubbleIcon />{p.comments?.length || 0}</div>
            </div>
          </div>
        ))}
      </div>
      ...
      {viewIndex !== null && (
        <div className="fixed inset-0 z-[100] bg-white md:bg-black/95 flex flex-col items-center overflow-y-auto">
          ...
          <div className="w-full max-w-[500px] flex flex-col gap-8 pb-20"><Post post={posts[viewIndex]} /></div>
        </div>
      )}
    </div>
  );
